...
Railsの学習ではリポジトリという概念が出てこなかったため、イメージしにくいかもしれません。   

Railsの場合、Javaのリポジトリに相当するものはあらかじめ用意されています。例えば、RailsでTweet.allのようにallメソッドを実行することで、Tweetsテーブルのデータ全件を取得することができました。   


これは、allメソッドを実行すると「select * from tweets」というSQLが発行されることがあらかじめ定義されているためです。   

このような定義はJavaの場合自分で実装する必要があります。その際使用するのがリポジトリです。   

## Lombok (ライブラリ)
- 定型文的なコードの記述を簡略化することができる
- @AllArgsConstructorは、コンストラクタを自動で生成してくれるアノテーション
- @Dataは、ゲッターとセッターの記述を省略することができるアノテーション
...
#  jarファイル(ジャーファイル)
Javaアプリの実行に必要な複数のファイルを１つにまとめた圧縮ファイルです。「Java Archive」の略称となっています。   
作成したファイルは「SampleApp.jar」のように拡張子に「.jar」がつく。このファイルはデプロイに必要なもの
